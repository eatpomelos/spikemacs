...
TARGET 是可选的标签列表，MUTEX 若为非 nil，则确保 TARGET 中的标签互斥。"
  ...
  (let* ((file (or (buffer-file-name)
    ...
    (let* ((current-kw (denote-extract-keywords-from-path file))
           ;; 修正：提供默认列表逻辑
           (actual-target (completing-read "Select state tag: " (or target spk-denote-class-keywords)))
...
(defun spk/find-today-journal-denote-entry ()
  "获取今天的日记（使用底层目录扫描，解决物理隔离路径返回空的问题）。"
  (interactive)
  (let* ((today-regexp (denote-journal--filename-date-regexp (current-time)))
         ;; 使用 Emacs 原生的 directory-files，不受 denote-directory 限制
         ;; 第三个参数 t 表示返回完整路径，最后一个参数 t 表示不排序（快）
         (all-files (directory-files denote-journal-directory t today-regexp t))
         ;; 进一步过滤掉目录，只保留文件
         (files (seq-filter (lambda (f) (not (file-directory-p f))) all-files))
