...
(setq spk-denote-state-keywords '("mustcheck" "archived" "permanent"))
(defconst spk-all-known-keywords (append denote-known-keywords spk-denote-state-keywords))
...
TARGET 是可选的标签列表，MUTEX 若为非 nil，则确保 TARGET 中的标签互斥。"
  ...
  (let* ((file (or (buffer-file-name)
    ...
    (let* ((current-kw (denote-extract-keywords-from-path file))
           ;; 修正：提供默认列表逻辑
           (actual-target (completing-read "Select state tag: " (or target denote-known-keywords)))
...
(defun spk/list-growing-cards ()
  "列出当前denote卡片中被标记为mustcheck的卡片"
  (interactive)
  (spk/list-cards-keyword "growing"))

(defun spk/list-keyword-cards ()
  (interactive)
  (if-let* ((kwd (completing-read "Input a keyword: " spk-all-known-keywords)))
...
(global-set-key (kbd "C-c n n") 'spk/list-mustcheck-cards)
(global-set-key (kbd "C-c n g") 'spk/list-growing-cards)
